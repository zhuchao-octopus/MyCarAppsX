...
public class OBD {
    ...
    private BluetoothSocket getBluetoothSocket(BluetoothDevice bluetoothDevice) {
        try {
            BluetoothSocket insecureSocket = bluetoothDevice.createInsecureRfcommSocketToServiceRecord(UUID.fromString(SPP_UUID));
            return insecureSocket;
        } catch (IOException e1) {
            Log.e(TAG, "Create Insecure socket failed" + e1.toString());
            try {
                BluetoothSocket secureSocket = bluetoothDevice.createRfcommSocketToServiceRecord(UUID.fromString(SPP_UUID));
                return secureSocket;
            } catch (Exception e) {
    ...
    private final String[] mOBDQuerInfoCmds = {
            OBD.CMD_Engine_coolant_temperature, OBD.CMD_Engine_RPM, OBD.CMD_Vehicle_speed,
    ...
    public void queryInfo() {
        if (mConnected && canQuerInfo) {
            canQuerInfo = false;
            sendCmds(mOBDQuerInfoCmds, 10000, 3, new Interface_SendCmdsCallback() {
                @Override
    ...
    private boolean connectBT() {
        ...
        if (mDeviceAddress == null || mDeviceAddress.isEmpty()) {
            ...
            if (ret == 1) {
                Log.d(TAG, "get default bonded address ok " + mDeviceName + ":" + mDeviceAddress + ":" + mBluetoothAdapter.getState());
                mHandler.sendMessage(mHandler.obtainMessage(101, "Find a default OBD device " + mDeviceName));
            } else if (ret == 2) {
                mHandler.sendMessage(mHandler.obtainMessage(101, "No OBDII bonded found"));
                return false;
            } else {
                mHandler.sendMessage(mHandler.obtainMessage(101, "No bonded address found"));
                return false;
            }
        }
        ...
        if (mDeviceAddress != null) {
            try {
                ...
                mRomoteDevice = mBluetoothAdapter.getRemoteDevice(mDeviceAddress);
                Log.d(TAG, "get remote device ok, addr:" + mRomoteDevice.getAddress() + " ,name:" + mRomoteDevice.getName() + " ,bondstate:" + mRomoteDevice.getBondState() + " ,type:"
                        //+ mRomoteDevice.getType() + " ,uuids:"
                        //	+ mRomoteDevice.getUuids()
                );
                if (mRomoteDevice.getAddress() == null || mRomoteDevice.getName() == null) {
                    Log.e(TAG, "get remote device addr and name error");
                    return false;
                }
                mBluetoothSocket = getBluetoothSocket(mRomoteDevice);
                if (mBluetoothSocket == null) return false;
        ...
        try {
            if (data != null && data.length > 0) {
                ...
                if (result.equals("STOPPED")) {
                    ...
                    if (result.startsWith(">")) notifyCmdRespToken();
                } else if (obd_recv_state == STATE_NORMAL) {
                    if (result.equals(">" + CMD_Engine_RPM) || result.equals(">" + CMD_Vehicle_speed) || result.equals(">" + CMD_Engine_coolant_temperature) || result.equals(">" + CMD_OBD_standards) || result.equals(">" + CMD_Short_term_fuel_trim1) || result.equals(">" + CMD_Long_term_fuel_trim1) || result.equals(">" + CMD_Short_term_fuel_trim2) || result.equals(">" + CMD_Long_term_fuel_trim2) || result.equals(">" + RESP_PIDs_supported1) || result.equals(">SEARCHING...")) {
                        bNextDataCanDeal = true;
                        return;
                    } else {    //AT@1 format
                        if (result.startsWith(">" + RESP_Engine_RPM) || result.startsWith(">" + RESP_Vehicle_speed) || result.startsWith(">" + RESP_Engine_coolant_temperature) || result.startsWith(">" + RESP_OBD_standards) || result.startsWith(">" + RESP_Short_term_fuel_trim1) || result.startsWith(">" + RESP_Long_term_fuel_trim1) || result.startsWith(">" + RESP_Short_term_fuel_trim2) || result.startsWith(">" + RESP_Long_term_fuel_trim2) || result.startsWith(">" + RESP_PIDs_supported1) || result.startsWith(">SEARCHING...")) {
                            bNextDataCanDeal = true;
                    ...
                    if (bNextDataCanDeal && result != null) {
                        ...
                        if (hexValue != null && hexValue.length >= 4) {
                            ...
                            } else if (result.startsWith(RESP_Engine_coolant_temperature)) {
                                mVehicleInfo.coolant_temperature = (hexValue[2] & 0xff) - 40;
                                notifyCmdRespToken();
                                mHandler.sendEmptyMessage(100);
                                Log.d(TAG, "COOLANT TEMP [" + mVehicleInfo.coolant_temperature + "]");
                            } else if (result.startsWith(RESP_Short_term_fuel_trim1)) {
                                calculateFuel(0, hexValue[2]);
                                mHandler.sendEmptyMessage(100);
                            } else if (result.startsWith(RESP_Long_term_fuel_trim1)) {
                                calculateFuel(1, hexValue[2]);
                                mHandler.sendEmptyMessage(100);
                            } else if (result.startsWith(RESP_Short_term_fuel_trim2)) {
                                calculateFuel(2, hexValue[2]);
                                mHandler.sendEmptyMessage(100);
                            } else if (result.startsWith(RESP_Long_term_fuel_trim2)) {
    ...
    private void receiver() {
        if (mInputStream != null) {
            ...
            try {
                ...
                while (true) {
                    int len = mInputStream.read(data, byteOffset, data.length - byteOffset);
                    if (len > 0) {
                        byteOffset += len;
                        ...
                        if (byteOffset > 0) {
                            ...
                            if (i < byteOffset) {
                                ...
                                // toHex(dataLine, i) + "]");
                                byteOffset -= (i + 1);
                                System.arraycopy(data, i + 1, data, 0, byteOffset);
    ...
    public void sendCmds(final String[] cmds, final int timeout, final int retryTimes, final Interface_SendCmdsCallback callback) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                    ...
                    synchronized (mCmdRespToken) {
                        try {
                            ...
                            if (mCmdRespToken.resp) {
                                ...
                                retry++;
                                Log.d(TAG, "sendCmds " + cmds[index] + " timeout," + " retry " + retry);
                            }
                        } catch (InterruptedException e) {
                            retry++;
                ...
                if (callback != null) callback.callback(index, null);
