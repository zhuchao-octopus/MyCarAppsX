 ...
 */
public class VideoUI extends UIBase implements OnGestureListener,
		OnDoubleTapListener, View.OnClickListener {
	...
	private static final int[] BUTTON_ON_CLICK = new int[] { R.id.prev,
			R.id.pp, R.id.next, R.id.repeat, R.id.btn_all, R.id.btn_local,
			R.id.btn_sd, R.id.btn_usb, R.id.btn_usb2, R.id.btn_usb3,
			R.id.btn_usb4, R.id.btn_usb4, R.id.full, R.id.btn_sd2,
			R.id.btn_add_all, R.id.repeat2, R.id.eq_mode_switch, R.id.list,
			R.id.full2,R.id.stop, R.id.eq};// ,R.id.mic,R.id.vol };

	private static final int[] VIEW_HIDE1 = new int[] { R.id.progress_bar,
			R.id.list_content, R.id.rightButtonLayout };
	...
	private static final int[][] VIEW_SHOW2 = new int[][] {
			//{ R.id.layout_visulizerView },
			{ 0 },
			{ R.id.list_content, R.id.rightButtonLayout, R.id.surface_view } };
	...
	public void updateFullUI() {
		if (mDisplayIndex == SCREEN1) {
			...
			if (type != mScreen1Type) {
				...
				if (mScreen1InitStatus == SCREEN1_STATUS_FULLSCREEN
						|| mIsFullScrean || type != TYPE_FULL_FUNCTION) {
...
//		 Log.d(TAG, "prepareFullScreen!!" + this);
	...
	private void changeToFullScreenEx() {
		if (mMediaPlayer != null && mMediaPlayer.isPlaying()) {
			...
			if (mMainView.findViewById(R.id.list_content) != null) {
				mMainView.findViewById(R.id.list_content).setVisibility(
						View.GONE);
			}

			if (mMainView.findViewById(R.id.list_content2) != null) {
				mMainView.findViewById(R.id.list_content2).setVisibility(
						View.GONE);
			}
			if (mMainView.findViewById(R.id.fake_black_cover_video) != null) {
				mMainView.findViewById(R.id.fake_black_cover_video)
						.setVisibility(View.GONE);
			}

			if (mMainView.findViewById(R.id.rightButtonLayout) != null) {
				mMainView.findViewById(R.id.rightButtonLayout).setVisibility(
						View.GONE);
			}
	...
	private void changeToSmallScreenEx() {
		...
		if (mMainView.findViewById(R.id.fake_black_cover_video) != null) {
			mMainView.findViewById(R.id.fake_black_cover_video).setVisibility(
					View.VISIBLE);
		}
		if (mMainView.findViewById(R.id.rightButtonLayout) != null) {
			mMainView.findViewById(R.id.rightButtonLayout).setVisibility(
...
//					updateView();
            ...
            if (mPlayingTVList != null && !update) {
                if (mPlayingTVList
                    ...
                    update = !mMediaPlayer
                            .equalCurrentPlaylist(((MyListViewAdapter) mPlayingTVList
                                    .getAdapter()).getList());
                }
            }
	...
	private void checkListHiglightItem(MyListViewAdapter la) {
		try {
			...
			String playing = ((MyListViewAdapter) mPlayingTVList.getAdapter())
					.getSelectName();
...
//					Log.d("abd", "doMemoryPlay f.exists():"+f.exists());
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case MSG_HIDE_MENU: {
				if ((System.currentTimeMillis() - mHomeBackHideTime) >= TIME_HIDE_MENU
						&& (mIsFullScrean)) {
					hideHomeBackKey();
				}
			}
		...
		if (mCurrentList != null && mMediaPlayer.getCurPosition() >= 0
			...
			if (mMediaPlayer.equalPreparePlaylist()
				...
				((MyListViewAdapter) mCurrentList.getAdapter())
						.setSelectItem(mMediaPlayer.getCurPosition()
								+ ((MyListViewAdapter) mCurrentList
										.getAdapter()).getFolderNum());

				if (((mCurrentList.getLastVisiblePosition()) < mMediaPlayer
						.getCurPosition())
						|| (mCurrentList.getFirstVisiblePosition() > mMediaPlayer
								.getCurPosition())) {
		...
		if (mDisplayIndex == 1) {
			...
			if (mUI[0] == null || mUI[0].mPause) {
				...
				mHandler.sendMessage(mHandler
						.obtainMessage(MSG_FIRST_RUN, 0, 150));
			} else {
...
//			mHandler.sendMessage(mHandler.obtainMessage(MSG_FIRST_RUN, 0, 5));
	...
	private Handler mIMediaCallBack = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case MusicPlayer.PLAYSTATE_CHANGED:
				updatePlayStatus();
				if (Util.isPX6() && Util.isAndroidQ())
					mHandler.sendEmptyMessageDelayed(MSG_UPDATE_PLAY_STATUS, 500);
				break;
	...
	private void hideHomeBackKey() {
		...
		mMainView.findViewById(R.id.bottom_button_layout).setVisibility(
				View.GONE);
	...
	private void showHomeBackKey() {
		...
		mHandler.sendMessageDelayed(
				mHandler.obtainMessage(MSG_HIDE_MENU, 0, 0), TIME_HIDE_MENU);
		...
		if (mDisplayIndex == SCREEN1) {
			...
			if (mScreen1Type == TYPE_FULL_FUNCTION) {
				...
				mMainView.findViewById(R.id.bottom_button_layout)
						.setVisibility(View.VISIBLE);
