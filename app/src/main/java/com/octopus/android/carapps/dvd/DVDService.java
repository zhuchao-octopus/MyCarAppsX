...
//		Log.d(TAG, mHandlerUICallBack[0] +"returnInfoToUI:"+what );
		for (int i = 0; i < mHandlerUICallBack.length; ++i) {
			if (mHandlerUICallBack[i] != null) {
				mHandlerUICallBack[i].sendMessageDelayed(mHandlerUICallBack[i]
						.obtainMessage(what, status, param, obj), 20);
			}
		}
	}
	...
	private void doDvdStatusChange(byte s) {
		if (mDvd.mDVDStatus != s) {
			if (s != MyCmd.Dvd.DVD_STATUS_DISK_INSIDE
					&& s != MyCmd.Dvd.DVD_STATUS_IN_ING) {
		...
		case MyCmd.Cmd.MCU_DVD_RECEIVE_DATA:
			...
			if (param != null && param.length >= 2) {
				...
				case 1:
					...
					returnInfoToUI(MyCmd.Cmd.APP_USER_PRIVATE,
							DVideoSpec.DVD_POWER_STATUS, mDvd.mDVDPower, null);
					break;
				case 2:
					doDvdStatusChange(param[2]);

					returnInfoToUI(MyCmd.Cmd.APP_USER_PRIVATE,
							DVideoSpec.DVD_DISK_STATUS, mDvd.mDVDStatus, null);
					break;
				}
