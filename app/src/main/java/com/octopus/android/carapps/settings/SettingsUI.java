...
public class SettingsUI extends UIBase implements View.OnClickListener {
	...
	private static final int[] BUTTON_ON_CLICK = new int[] {
			R.id.screen_size_title, R.id.switch_screen,
			R.id.screen_view_select, R.id.wallpaper_instructions,R.id.backlight_screen1 };
	...
	private static final int[] VIEW_TYPE1 = new int[] { R.id.screen_size_title,
		/*R.id.switch_screen*/ };
        ...
        if (id == R.id.screen_size_title) {
            ...
            try {
                ...
                it.setClassName("com.car.ui",
                        "com.my.wallpaper.WallpaperChooser");
	...
	private final DialogInterface.OnClickListener mClickListenerSwitch = new DialogInterface.OnClickListener() {
		public void onClick(DialogInterface dialog, int whichButton) {
			BroadcastUtil.sendToCarService(mContext,
					MyCmd.Cmd.APP_REQUEST_SEND_KEY, MyCmd.Keycode.POWER);
...
//			Util.setFileValue(SCREEN_SWITCH, i);
	...
	private void updateScreenSize() {
		...
		String[] array = mContext.getResources().getStringArray(
				R.array.screen_size_select);
		((TextView) mMainView.findViewById(R.id.screen_size_value))
				.setText(array[mSelectScreenSize]);
	...
	private void showScreenDialog() {
		...
		Dialog d = new AlertDialog.Builder(mContext)
				...
				.setTitle(R.string.screen_size_title)
				.setSingleChoiceItems(R.array.screen_size_select,
						mSelectScreenSize, new OnClickListener() {
							@Override
	...
	private void showScreenViewDialog() {
		...
		if (mDisplayIndex == 0) {
			...
			.setTitle(R.string.screen_view_select)
			.setMultiChoiceItems(R.array.screen1_view_select,
					mCheckedItemsTemp, new OnMultiChoiceClickListener() {
						@Override
						public void onClick(DialogInterface dialog,
								int which, boolean b) {
			...
			.setPositiveButton(android.R.string.ok, mClickListenerScreenview
	...
	private void updateScreen1Setting() {
		...
		((TextView) mMainView.findViewById(R.id.screen_view_select_value))
				.setText(s);
	...
	private void setScreen1Setting() {
		String[] array = mContext.getResources().getStringArray(
				R.array.screen1_view_select_value);
		String s = "";
		for (int i = 0; i < mCheckedItems.length; ++i) {
			if (mCheckedItems[i]) {
				if (i < array.length) {
