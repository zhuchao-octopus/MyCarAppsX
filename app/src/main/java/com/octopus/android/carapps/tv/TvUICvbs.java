...
public class TvUICvbs extends UIBase implements View.OnClickListener, SurfaceHolder.Callback {
    ...
    public static final int SOURCE = MyCmd.SOURCE_DTV_CVBS;
    public static TvUICvbs[] mUI = new TvUICvbs[MAX_DISPLAY];

    public static TvUICvbs getInstanse(Context context, View view, int index) {
        if (index >= MAX_DISPLAY) {
            return null;
        }

        mUI[index] = new TvUICvbs(context, view, index);
        return mUI[index];
    }

    public TvUICvbs(Context context, View view, int index) {
    ...
    public void onCreate() {
        ...
        mBrakeCarView = (TextView) mMainView.findViewById(R.id.brake_warning_text);
        if (MachineConfig.VALUE_SYSTEM_UI_KLD7_1992.equals(GlobalDef.getSystemUI())) {
            mBrakeCarView.setText(R.string.warning_driving_1992);
        }
        //else if (MachineConfig.VALUE_SYSTEM_UI22_1050.equals(GlobalDef.getSystemUI())){
        //mBrakeCarView.setText(R.string.warning_driving_1050);
        //}
        else {
            ...
            if (f.exists()) {
                ...
                if (d != null) {
                    mBrakeCarView.setText("");
                    mBrakeCarView.setBackground(d);
                }
            }
        }
    ...
    private void toggleKeyboard() {
        View v = mMainView.findViewById(R.id.radio_keyboard_main);
        if (v != null) {
    ...
    private void sendCmd(int cmd) {
        Log.d(TAG, "sendCmd:" + Integer.toHexString(cmd));
        cmd |= 0x4000000;
        BroadcastUtil.sendToCarService(mContext, MyCmd.Cmd.SET_DTV_CMD, cmd);
    }

    private void sendCmdEx(int cmd) {
        Log.d(TAG, "sendCmdEx:" + Integer.toHexString(cmd));
    ...
    public void reverseStop() {
        ...
        GlobalDef.setCameraSource(MyCmd.CAMERA_SOURCE_DTV_CVBS);
    }
    ...
    public void onPause() {
        ...
        mMainView.findViewById(R.id.only_black).setVisibility(View.VISIBLE);
        TVService.setUICallBack(null, mDisplayIndex);
        stopCheckSignal();
        stopCheckBrakeCar();
        mPreSignal = -1;
        mSignal = -1;
    ...
    public void onResume() {
        ...
        TVService.setUICallBack(mHandler, mDisplayIndex);
    ...
    private TextView mBrakeCarView;
