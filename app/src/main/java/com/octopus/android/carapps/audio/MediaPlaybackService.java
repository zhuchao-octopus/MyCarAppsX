...
public class MediaPlaybackService extends ServiceBase {
	...
	public static MediaPlaybackService getInstanse(Context context) {
		if (mThis == null) {
...
//		}
	...
	private static ComMediaPlayer.IMediaCallBack mIMediaCallBack = new ComMediaPlayer.IMediaCallBack() {
		public void callback(int what, int status, Object obj) {
			...
			if (what == MusicPlayer.STORAGE_MOUNTED) {
				if (mHandlerUICallBack[0] != null) {
					mHandlerUICallBack[0].sendMessageDelayed(
							mHandlerUICallBack[0].obtainMessage(what, status,
									0, obj), 20);
				} else if (mThis != null) {
					if (Util.isRKSystem()){
						
						MusicUI mUI = MusicUI.getInstanse(0);
						if (mUI!=null){
				...
				for (int i = 0; i < mHandlerUICallBack.length; ++i) {
					if (mHandlerUICallBack[i] != null) {
						mHandlerUICallBack[i].sendMessageDelayed(
								mHandlerUICallBack[i].obtainMessage(what,
										status, 0, obj), 20);
					}
				}
			}
	...
	private void doStorageMounted(int page) {
		...
		boolean open = false;
		boolean app0Show = (MusicUI.getInstanse(0) != null && !MusicUI
				.getInstanse(0).mPause);

		boolean app1Show = (MusicUI.getInstanse(1) != null && !MusicUI
				.getInstanse(1).mPause);

		// GlobalDef.mAutoPlayMusicWhenStorageMounted
		try {
			int i = Settings.Global.getInt(mContext.getContentResolver(),
					SystemConfig.AUTO_PLAY_MUSIC_DEVICES_MOUNTED);
	...
	private static void doStorageEject() {
		...
		Log.d("abc", "BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB" + MusicUI.getInstanse(0));
		if ((MusicUI.getInstanse(0) != null && !MusicUI.getInstanse(0).mPause)) {
			MusicActivity.finishActivity(GlobalDef.mContext);
			BroadcastUtil.sendToCarServiceSetSource(GlobalDef.mContext,
					MyCmd.SOURCE_MX51);
			Log.d("abc", "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA");
			
		} else if ((MusicUI.getInstanse(1) != null && !MusicUI.getInstanse(1).mPause)) {
					...
					if (mMediaPlayer != null && mMediaPlayer.isInitialized()) {
						if (GlobalDef.mCanboxNeedPlayInfo) {
							BroadcastUtil
									.sendCanboxInfo(
											mContext,
											MyCmd.AUDIO_SOURCE_CHANGE,
											mMediaPlayer.getCurPosition(),
											(int) (mMediaPlayer
													.getCurrentPosition() / 1000L),
											mMediaPlayer.getCurrentTotal()
									);
						...
						if (GlobalDef.mIsMediaWidget) {
							Intent i = new Intent(
									MyCmd.BROADCAST_CMD_FROM_MUSIC);
							i.putExtra(MyCmd.EXTRA_COMMON_CMD,
									MyCmd.Cmd.MUSIC_SEND_PLAY_TIME);

							i.putExtra(MyCmd.EXTRA_COMMON_DATA2,
									mMediaPlayer.getCurrentPosition());
							i.putExtra(MyCmd.EXTRA_COMMON_DATA3,
			...
			case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
				if (!GlobalDef.isAudioFocusGPS()) {
					...
					if (GlobalDef.mSource == MyCmd.SOURCE_MUSIC) {
						...
						BroadcastUtil.sendToCarServiceSetSource(
								GlobalDef.mContext, MyCmd.SOURCE_OTHERS_APPS);
					}
				}
				break;
