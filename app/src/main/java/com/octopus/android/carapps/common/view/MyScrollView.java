...
public class MyScrollView extends HorizontalScrollView {
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if (downScrollX == -1) {
            ...
            Log.d("ddd", time + ":" + l);
            if (time > 800) {
                if (mWholdePage == -1 && mDefaultPage >= 0 && mDefaultPage < PAGE.length) {
    ...
    protected void onLayout(boolean arg0, int arg1, int arg2, int arg3, int arg4) {
        ...
        //		Log.d("aabb", "onLayout:");
        if (mDefaultPage != -1 && mDefaultPage >= 0 && mDefaultPage < PAGE.length) {
            mPage = mDefaultPage;
    ...
    private void showPageIndex(int index) {
        ...
        // }
        // }
        if (index >= 0 && index < PAGE.length) mWholdePage = PAGE[index];
    }
