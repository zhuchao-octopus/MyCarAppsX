package com.octopus.android.carapps.car.service;
...
public class UIService extends Service {
	...
	com.octopus.android.carapps.audio.MediaPlaybackService mMediaPlaybackService;
	...
	public void onCreate() {
		...
		mMediaPlaybackService = com.octopus.android.carapps.audio.MediaPlaybackService.getInstanse(this);
	...
	private void updateSource(int cmd, Intent intent) {
		...
		if (GlobalDef.mSource != source && mServiceBase != null) {
			if (((mServiceBase.getSource() == GlobalDef.mSource) && source != MyCmd.SOURCE_OTHERS_APPS)
					|| (GlobalDef.mSource == MyCmd.SOURCE_OTHERS_APPS && mServiceBase.getSource() != source)) {
				// move to GlobalDef.
		...
		case MyCmd.SOURCE_MUSIC:
			mServiceBase = com.octopus.android.carapps.audio.MediaPlaybackService.getInstanse(this);
			mServiceBase.doEQResule();
			break;
	...
	private void doKeyControl(int code) {
		...
		case MyCmd.Keycode.RADIO_POWER: {
			if (GlobalDef.mSource != RadioUI.SOURCE) {
				...
				GlobalDef.reactiveSource(this, RadioUI.SOURCE,
				...
				public void onReceive(Context context, Intent intent) {
					...
					if (action.equals(MyCmd.BROADCAST_CAR_SERVICE_SEND)) {
						...
						case MyCmd.Cmd.REVERSE_STATUS:
							GlobalDef.mReverseStatus = intent.getIntExtra(
							...
							try {
								...
								Log.e(TAG,
										"mServiceBase instanceof AuxInService err");
							}
							break;
						...
						case MyCmd.Cmd.REQUEST_SCREEN1_SHOW:
							...
							Log.d("allen1", "!!!!!!!!" + source);
							if (mPresentationUI != null
									&& source != MyCmd.SOURCE_OTHERS_APPS) {
								mPresentationUI.update(source);
							}
							break;
						...
						if (GlobalDef.mSource == MyCmd.SOURCE_MUSIC) {
							if (mMediaPlaybackService != null) {
								mMediaPlaybackService.doCmd(
										MyCmd.Cmd.MUSIC_SEND_PLAY_STATUS, intent);
							}
						}
						...
						if (MachineConfig.KEY_RDS.equals(s)) {
							MachineConfig
									.getPropertyOnce(MachineConfig.KEY_RDS);
							...
							.equals(MyCmd.BROADCAST_ACC_DELAY_POWER_OFF)) {
						GlobalDef.mPowerOffTime = SystemClock.uptimeMillis();
						if (mMediaPlaybackService != null) {
							mMediaPlaybackService.doCmd(
									MyCmd.Cmd.ACC_POWER_OFF, intent);
						}
						if (mVideoService != null) {
							mVideoService
									.doCmd(MyCmd.Cmd.ACC_POWER_OFF, intent);
						}
						if (mDVDService != null) {
						...
						ParkBrake.saveCameraStatasIfSleep();
							...
							.equals(MyCmd.BROADCAST_ACC_DELAY_POWER_ON)) {
						Log.d("MusicPlayer", "GlobalDef.mSleepOnTime1:"
						...
						if (Util.isRKSystem()) {
							...
							if (mMediaPlaybackService != null) {
								mMediaPlaybackService.doCmd(
										MyCmd.Cmd.ACC_POWER_ON, intent);
							}
							if (mVideoService != null) {
								mVideoService.doCmd(MyCmd.Cmd.ACC_POWER_ON,
										intent);
							}
						}
			...
			mReceiverWidgetKey = new BroadcastReceiver() {
				...
				public void onReceive(Context context, Intent intent) {
					...
					try{
						...
						if (GlobalDef.mSource == source
								|| code == MyCmd.Keycode.RADIO_POWER) {
							doKeyControl(code);
						}
					} catch(Exception e){
