...
public class MusicUI extends UIBase implements View.OnClickListener {
	...
	private static MusicUI[] mUI = new MusicUI[MAX_DISPLAY];

	public static MusicUI getInstanse(Context context, View view, int index) {
	...
	public static MusicUI getInstanse(int index) {
		if (index >= MAX_DISPLAY) {
			return null;
		}
		return mUI[index];
	...
	private static final int[] BUTTON_ON_CLICK = new int[] { R.id.back,
			...
			R.id.repeat2, R.id.btn_folder, R.id.album_and_visualizer_view,
			R.id.list, R.id.eq_mode_switch, R.id.id3, R.id.folder_switch,
			R.id.play_repeat_tag};//,R.id.mic,R.id.vol };
	...
	private static final int[] VIEW_HIDE2 = new int[] {
			R.id.bottom_button_layout, R.id.list_content, R.id.seek_bar_layout,
			R.id.tb };
	...
	private void checkIfNeedUpdatePlayingList() {// only display1 show
		...
		ArrayList<String> list = mMediaPlayer.getStrList(
				MusicPlayer.LIST_PLAYING, -1);
		if (list.size() > 0) {
			MyListViewAdapter ap = (MyListViewAdapter) mPlayingTVList
					.getAdapter();
	...
	public void onCreate() {
		...
		mViewFolderListviewSame = mMainView
				.findViewById(R.id.folder_listview_same_layout);
...
//		mAlbumArt2 = (ImageView) mMainView.findViewById(R.id.album_art2);
		mVisualizerView = (VisualizerView) mMainView
				.findViewById(R.id.visualizer_view);
	...
	private void initLyc() {
		...
		if (v != null) {
			...
			int lines = mMainView.getResources()
					.getInteger(R.integer.lrc_lines);
			if (lines == 3) { // for line3
				mLrcview.setLines(
						lines,
						mMainView.getResources().getInteger(
								R.integer.lrc_margin_top));
						...
						if (lrcViewHeight <= 0) {
							...
							if (mLrcview.lines == 3) {
								if (lrcViewHeight > 0) {
									...
									float curline_scaling = (float) mMainView
											.getResources()
											.getInteger(
													R.integer.lrc_curline_scaling) / 100.0f;
									float otherline_scaling = (float) mMainView
											.getResources()
											.getInteger(
													R.integer.lrc_otherline_scaling) / 100.0f;
									mLrcview.setLrcTextSizePixel(
											(int) (h * curline_scaling), true);
									mLrcview.setLrcTextSizePixel(
											(int) (h * otherline_scaling),
											false);
									int curlineColor = mMainView
											.getResources()
											.getInteger(
													R.integer.lrc_curline_color);
									int otherlineColor = mMainView
											.getResources()
											.getInteger(
													R.integer.lrc_otherline_color);
									mLrcview.setLcrTextPrevNextColor(Color.rgb(
											otherlineColor >> 16 & 0xff,
											otherlineColor >> 8 & 0xff,
											otherlineColor & 0xff));
									mLrcview.setFirstColor(Color.rgb(
											curlineColor >> 16 & 0xff,
											curlineColor >> 8 & 0xff,
											curlineColor & 0xff));
								}
							}
			...
			int size = mContext.getResources().getInteger(
					R.integer.lrc_text_size);
			...
			if (MachineConfig.VALUE_SYSTEM_UI21_RM12.equals(GlobalDef
					.getSystemUI())) {
				mID3View = ID3_LRC;
			} else if (MachineConfig.VALUE_SYSTEM_UI45_8702_2.equals(GlobalDef
					.getSystemUI())
					|| MachineConfig.VALUE_SYSTEM_UI43_3300_1.equals(GlobalDef
							.getSystemUI())) {
				ID3_NUM = 3;
		...
		public void callback(int cmd, int visible_page) {
			...
			if (cmd == MyScrollView.CMD_NEWPAGE) {
				...
				if (visible_page == 0) {
					...
					mMainView.findViewById(R.id.tv_all_list).setVisibility(
							View.VISIBLE);
					...
					if (MachineConfig.VALUE_SYSTEM_UI20_RM10_1.equals(GlobalDef
							...
							|| MachineConfig.VALUE_SYSTEM_UI43_3300_1
									.equals(GlobalDef.getSystemUI())) {
						doSetPage(R.id.btn_all);
						...
						if (scrollToPlayingPage) {
							...
							if (mContext instanceof Activity) {
								View vf = ((Activity) mContext)
										.getCurrentFocus();
								if (vf != null) {
									mMainView.findViewById(R.id.tv_all_list)
											.requestFocus();
								}
							}
						}
	...
	private void initListView(int id) {
        ...
        if (id == R.id.tv_all_list) {
            if (mPlayingTVList == null) {
                mPlayingTVList = (ListView) mMainView
                        .findViewById(R.id.tv_all_list);
                mPlayingTVList
                            ...
                            public void onItemClick(AdapterView<?> arg0,
                                ...
                                GlobalDef
                                        .reactiveSource(
                                                mContext,
                                                SOURCE,
                                                com.octopus.android.carapps.audio.MediaPlaybackService.mAudioFocusListener);
            ...
            if (mLocalTVList == null) {
                ArrayList<String> list = mMediaPlayer.getStrList(
                        MusicPlayer.LIST_LOCAL, -1);
                if (list.size() > 0) {
                    mLocalTVList = (ListView) mMainView
                            .findViewById(R.id.tv_local_list);
                    updateListViewAdapter(mLocalTVList, list,
                            mMediaPlayer
                                    .getStrFolderNum(MusicPlayer.LIST_LOCAL));
                    mLocalTVList
                            .setOnItemClickListener(new OnItemClickListener() {
                                public void onItemClick(AdapterView<?> arg0,
                                                        View view, int postion, long id) {
                                    int folderNum = ((MyListViewAdapter) arg0
                                            .getAdapter()).getFolderNum();
                                    if (postion < folderNum) {
                                        clickFolder(mLocalTVList,
                                                MusicPlayer.LIST_LOCAL,
                                                postion,
                                                ((MyListViewAdapter) arg0
                                                        .getAdapter())
                                                        .getFolderLevel());
                                    } else {
                ...
                mMediaPlayer.getStrList(MusicPlayer.LIST_LOCAL,
                        ((MyListViewAdapter) mLocalTVList.getAdapter())
                                .getFolderSet());
                ((MyListViewAdapter) mLocalTVList.getAdapter())
                        .clearSelectItem();
            }
            ...
            if (mUsbTVList == null) {
                ArrayList<String> list = mMediaPlayer.getStrList(
                        MusicPlayer.LIST_USB, -1);

                if (list.size() > 0) {
                    mUsbTVList = (ListView) mMainView
                            .findViewById(R.id.tv_usb_list);
                    updateListViewAdapter(mUsbTVList, list,
                            mMediaPlayer.getStrFolderNum(MusicPlayer.LIST_USB));
                            ...
                            .setOnItemClickListener(new OnItemClickListener() {
                                public void onItemClick(AdapterView<?> arg0,
                                    ...
                                    int folderNum = ((MyListViewAdapter) arg0
                                            .getAdapter()).getFolderNum();
                                    if (postion < folderNum) {
                                        clickFolder(mUsbTVList,
                                                MusicPlayer.LIST_USB, postion,
                                                ((MyListViewAdapter) arg0
                                                        .getAdapter())
                                                        .getFolderLevel());
                                    } else {
                ...
                mMediaPlayer.getStrList(MusicPlayer.LIST_USB,
                        ((MyListViewAdapter) mUsbTVList.getAdapter())
                                .getFolderSet());
            ...
            if (mUsb2TVList == null) {
                ArrayList<String> list = mMediaPlayer.getStrList(
                        MusicPlayer.LIST_USB2, -1);

                if (list.size() > 0) {
                    mUsb2TVList = (ListView) mMainView
                            .findViewById(R.id.tv_usb2_list);
                    updateListViewAdapter(mUsb2TVList, list,
                            mMediaPlayer.getStrFolderNum(MusicPlayer.LIST_USB2));
                            ...
                            .setOnItemClickListener(new OnItemClickListener() {
                                public void onItemClick(AdapterView<?> arg0,
                                    ...
                                    int folderNum = ((MyListViewAdapter) arg0
                                            .getAdapter()).getFolderNum();
                                    if (postion < folderNum) {
                                        clickFolder(mUsb2TVList,
                                                MusicPlayer.LIST_USB2, postion,
                                                ((MyListViewAdapter) arg0
                                                        .getAdapter())
                                                        .getFolderLevel());
                                    } else {
                ...
                mMediaPlayer.getStrList(MusicPlayer.LIST_USB2,
                        ((MyListViewAdapter) mUsb2TVList.getAdapter())
                                .getFolderSet());
                ((MyListViewAdapter) mUsb2TVList.getAdapter())
                        .clearSelectItem();
            }
            ...
            if (mUsb3TVList == null) {
                ArrayList<String> list = mMediaPlayer.getStrList(
                        MusicPlayer.LIST_USB3, -1);

                if (list.size() > 0) {
                    mUsb3TVList = (ListView) mMainView
                            .findViewById(R.id.tv_usb3_list);
                    updateListViewAdapter(mUsb3TVList, list,
                            mMediaPlayer.getStrFolderNum(MusicPlayer.LIST_USB3));
                            ...
                            .setOnItemClickListener(new OnItemClickListener() {
                                public void onItemClick(AdapterView<?> arg0,
                                    ...
                                    int folderNum = ((MyListViewAdapter) arg0
                                            .getAdapter()).getFolderNum();
                                    if (postion < folderNum) {
                                        clickFolder(mUsb3TVList,
                                                MusicPlayer.LIST_USB3, postion,
                                                ((MyListViewAdapter) arg0
                                                        .getAdapter())
                                                        .getFolderLevel());
                                    } else {
                ...
                mMediaPlayer.getStrList(MusicPlayer.LIST_USB3,
                        ((MyListViewAdapter) mUsb3TVList.getAdapter())
                                .getFolderSet());
                ((MyListViewAdapter) mUsb3TVList.getAdapter())
                        .clearSelectItem();
            }
            ...
            if (mUsb4TVList == null) {
                ArrayList<String> list = mMediaPlayer.getStrList(
                        MusicPlayer.LIST_USB4, -1);

                if (list.size() > 0) {
                    mUsb4TVList = (ListView) mMainView
                            .findViewById(R.id.tv_usb4_list);
                    updateListViewAdapter(mUsb4TVList, list,
                            mMediaPlayer.getStrFolderNum(MusicPlayer.LIST_USB4));
                            ...
                            .setOnItemClickListener(new OnItemClickListener() {
                                public void onItemClick(AdapterView<?> arg0,
                                    ...
                                    int folderNum = ((MyListViewAdapter) arg0
                                            .getAdapter()).getFolderNum();
                                    if (postion < folderNum) {
                                        clickFolder(mUsb4TVList,
                                                MusicPlayer.LIST_USB4, postion,
                                                ((MyListViewAdapter) arg0
                                                        .getAdapter())
                                                        .getFolderLevel());
                                    } else {
                ...
                mMediaPlayer.getStrList(MusicPlayer.LIST_USB4,
                        ((MyListViewAdapter) mUsb4TVList.getAdapter())
                                .getFolderSet());
                ((MyListViewAdapter) mUsb4TVList.getAdapter())
                        .clearSelectItem();
            }
            ...
            if (mSDTVList == null) {
                ArrayList<String> list = mMediaPlayer.getStrList(
                        MusicPlayer.LIST_SD, -1);
                if (list.size() > 0) {
                    mSDTVList = (ListView) mMainView
                            .findViewById(R.id.tv_sd_list);

                    updateListViewAdapter(mSDTVList, list,
                            mMediaPlayer.getStrFolderNum(MusicPlayer.LIST_SD));
                    ...
                    mSDTVList.setOnItemClickListener(new OnItemClickListener() {
                        public void onItemClick(AdapterView<?> arg0, View view,
                            ...
                            int folderNum = ((MyListViewAdapter) arg0
                                    .getAdapter()).getFolderNum();
                            if (postion < folderNum) {
                                clickFolder(mSDTVList, MusicPlayer.LIST_SD,
                                        postion, ((MyListViewAdapter) arg0
                                                .getAdapter()).getFolderLevel());
                            } else {
                                play(postion - folderNum);
                                toPlayingIfFolderUI();
                            }
                ...
                mMediaPlayer.getStrList(MusicPlayer.LIST_SD,
                        ((MyListViewAdapter) mSDTVList.getAdapter())
                                .getFolderSet());
            ...
            if (mSD2TVList == null) {
                ArrayList<String> list = mMediaPlayer.getStrList(
                        MusicPlayer.LIST_SD2, -1);
                if (list.size() > 0) {
                    mSD2TVList = (ListView) mMainView
                            .findViewById(R.id.tv_sd2_list);

                    updateListViewAdapter(mSD2TVList, list,
                            mMediaPlayer.getStrFolderNum(MusicPlayer.LIST_SD2));
                            ...
                            .setOnItemClickListener(new OnItemClickListener() {
                                public void onItemClick(AdapterView<?> arg0,
                                    ...
                                    int folderNum = ((MyListViewAdapter) arg0
                                            .getAdapter()).getFolderNum();
                                    if (postion < folderNum) {
                                        clickFolder(mSD2TVList,
                                                MusicPlayer.LIST_SD2, postion,
                                                ((MyListViewAdapter) arg0
                                                        .getAdapter())
                                                        .getFolderLevel());
                                    } else {
                ...
                mMediaPlayer.getStrList(MusicPlayer.LIST_SD2,
                        ((MyListViewAdapter) mSD2TVList.getAdapter())
                                .getFolderSet());
            ...
            if (mAllFoderList == null) {
                ArrayList<String> list = mMediaPlayer.getStrList(
                        MusicPlayer.LIST_ALL_FOLDER, -1);
                if (list.size() > 0) {
                    mAllFoderList = (ListView) mMainView
                            .findViewById(R.id.tv_all_folder_list);

                    updateListViewAdapter(
                            mAllFoderList,
                            list,
                            mMediaPlayer
                                    .getStrFolderNum(MusicPlayer.LIST_ALL_FOLDER));
                            ...
                            .setOnItemClickListener(new OnItemClickListener() {
                                public void onItemClick(AdapterView<?> arg0,
                                    ...
                                    int folderNum = ((MyListViewAdapter) arg0
                                            .getAdapter()).getFolderNum();
                                    if (postion < folderNum) {
                                        clickFolder(mAllFoderList,
                                                MusicPlayer.LIST_ALL_FOLDER,
                                                postion,
                                                ((MyListViewAdapter) arg0
                                                        .getAdapter())
                                                        .getFolderLevel());
                ...
                mMediaPlayer.getStrList(MusicPlayer.LIST_ALL_FOLDER,
                        ((MyListViewAdapter) mAllFoderList.getAdapter())
                                .getFolderSet());
                ((MyListViewAdapter) mAllFoderList.getAdapter())
                        .clearSelectItem();
            }
	...
	private void updateListViewAdapter(ListView lv, ArrayList<String> list,
			int folder) {
		if (lv != null) {
			int id = 0;
			if (mDisplayIndex == 0) {
			...
			MyListViewAdapter ad = new MyListViewAdapter(mContext, id, list,
					folder);
			lv.setAdapter(ad);
			...
			if (MachineConfig.VALUE_SYSTEM_UI45_8702_2.equals(GlobalDef
				...
				ad.updateId3Time(al);
				ad.setPlayingType(1);
				ad.setSelectTextColor(mContext.getResources().getColor(
	...
	private void clickFolder(ListView lv, int id, int index, int level) {
		if (level == 1) {
			...
			updateListViewAdapter(lv, mMediaPlayer.getStrList(id, -1),
					mMediaPlayer.getStrFolderNum(id));
		}
	}
	...
	private void doMemoryPlay() {
		...
		if (mMediaPlayer.isInitialized()) {
			...
			// }
			Log.d("abd", mMediaPlayer.mSaveForSleepPath + "doMemoryPlay:"
					+ mFirstPowerOn);
			...
			if ((GlobalDef.mSourceWillUpdate == SOURCE
					|| GlobalDef.mSource == SOURCE || GlobalDef.mSource == MyCmd.SOURCE_NONE)
					&& (!mPause || mGpsRunAfter)) {
		...
		if (mDisplayIndex == 0) {
			if (mCreate) {
				...
				// if (mPlayingTVList.getAdapter() == null) {
				updateListViewAdapter(mPlayingTVList,
						mMediaPlayer.getStrList(MusicPlayer.LIST_PLAYING, -1),
						0);
				// }
			}
		} else {
			...
			// if (mPlayingTVList.getAdapter() == null) {
			updateListViewAdapter(mPlayingTVList,
					mMediaPlayer.getStrList(MusicPlayer.LIST_PLAYING, -1), 0);
		}
	...
	private void doFirstRunAsync() {
		...
		ArrayList<String> list = mMediaPlayer.getStrListEx(
				MusicPlayer.LIST_PLAYING, -1);
			...
			if (list.size() > 0) {
				...
				if (list.size() > 0) {
					...
					if (list.size() > 0) {
						...
						list = mMediaPlayer.getStrListEx(MusicPlayer.LIST_USB2,
								-1);
						if (list.size() > 0) {
							updateNewActivity(MusicPlayer.LIST_USB2);
						} else {
							list = mMediaPlayer.getStrListEx(
									MusicPlayer.LIST_USB3, -1);
							if (list.size() > 0) {
	...
	private void doSetPage(int id) {
		if (mFolderType == FOLDER_TYPE_FOLDER) {
			setViewVisible(mMainView, R.id.folder_listview_same_layout,
					View.GONE);
        ...
        if (id == R.id.btn_local) {
            ...
            mMainView.findViewById(R.id.tv_local_list).setVisibility(
                    View.VISIBLE);
            ...
            mMainView.findViewById(R.id.btn_add_all)
            ...
            mMainView.findViewById(R.id.tv_local_list).setVisibility(View.GONE);
            mMainView.findViewById(R.id.tv_sd2_list)
                    .setVisibility(View.VISIBLE);
            mMainView.findViewById(R.id.tv_sd_list).setVisibility(View.GONE);
            mMainView.findViewById(R.id.tv_usb_list).setVisibility(View.GONE);
            ...
            mMainView.findViewById(R.id.btn_add_all)
                    .setVisibility(View.VISIBLE);
            mMainView.findViewById(R.id.tv_usb2_list).setVisibility(
                    View.VISIBLE);
            ...
            mMainView.findViewById(R.id.btn_add_all)
                    .setVisibility(View.VISIBLE);
            mMainView.findViewById(R.id.tv_usb3_list).setVisibility(
                    View.VISIBLE);
            ...
            mMainView.findViewById(R.id.btn_add_all)
                    .setVisibility(View.VISIBLE);
            mMainView.findViewById(R.id.tv_usb4_list).setVisibility(
                    View.VISIBLE);
            ...
            mMainView.findViewById(R.id.btn_add_all)
                    .setVisibility(View.VISIBLE);
            mMainView.findViewById(R.id.tv_usb_list)
                    .setVisibility(View.VISIBLE);
            ...
            if (!mIs3PageSroll) {
                ...
                mMainView.findViewById(R.id.tv_local_list).setVisibility(
                        View.GONE);
                mMainView.findViewById(R.id.tv_sd_list)
                        .setVisibility(View.GONE);
                mMainView.findViewById(R.id.tv_sd2_list).setVisibility(
                        View.GONE);
                mMainView.findViewById(R.id.tv_usb_list).setVisibility(
                        View.GONE);
                ...
                mMainView.findViewById(R.id.tv_usb4_list).setVisibility(
                        View.GONE);
                mMainView.findViewById(R.id.btn_add_all).setVisibility(
                        View.GONE);
            } else {
                if (MachineConfig.VALUE_SYSTEM_UI20_RM10_1.equals(GlobalDef
                        .getSystemUI())
		...
		if (v != null) {
			...
			if (GlobalDef.checkStorageIsMounted(path)) {
				...
				if (Util.isRKSystem()) {
					...
					} else if (R.id.btn_usb == id || R.id.btn_usb2 == id
							|| R.id.btn_usb3 == id || R.id.btn_usb4 == id) {
	...
	private void resetVisualizer() {
		...
		try {
			if (mMediaPlayer.isPlaying()) {
				...
				if (mMediaPlayer.getAudioSessionId() == -1
				...
				mVisualizer.setDataCaptureListener(mVisualizerView,
...
//				mAlbumArt.setScaleType(ImageView.ScaleType.FIT_CENTER);
	...
	private void updateView() {
		...
		updateStorageView();
		if (mCurrentList != null && mMediaPlayer.getCurPosition() >= 0
				&& mMediaPlayer.getCurPosition() < mCurrentList.getCount()
				&& mCurrentList.getAdapter() != null) {

			int playPos = mMediaPlayer.getCurPosition()
					+ ((MyListViewAdapter) mCurrentList.getAdapter())
							.getFolderNum();
			if (mMediaPlayer.equalPreparePlaylist()
					|| mCurrentList == mPlayingTVList) {
				((MyListViewAdapter) mCurrentList.getAdapter())
						.setSelectItem(playPos);

				if (((mCurrentList.getLastVisiblePosition()) < playPos)
						|| (mCurrentList.getFirstVisiblePosition() > playPos)) {
		...
		if (v != null) {
			...
			if (mMediaPlayer != null && mMediaPlayer.isInitialized()
					&& mMediaPlayer.getCurSongNum() > 0) {
				s = (1 + mMediaPlayer.getCurPosition()) + "/"
						+ mMediaPlayer.getCurSongNum();
			}
				...
				if (mMediaPlayer != null) {
					...
					if (TrackPath != null) {
						...
						if (f.exists()) {
							...
							if (lrclist != null) {
								mLrcview.setLyric(new LrcPackage(lrclist,
										(int) mMediaPlayer.getDuration()));
								mLrcview.initLrcIndex((int) mMediaPlayer
										.getCurrentPosition());
								mLrcview.setVisibility(View.VISIBLE);
				...
				if (Util.isRKSystem()) {
					...
					if (drawable_id != 0) {
						Drawable d = mContext.getResources().getDrawable(
								drawable_id);
						if (d != null) {
							ImageView iv = (ImageView) mMainView
									.findViewById(R.id.iv_media_path);
							if (iv != null) {
        ...
        if (id == R.id.back) {
            ...
            if (mMediaPlayer.isPlaying()) {
                ...
                GlobalDef.reactiveSource(mContext, SOURCE,
                        MediaPlaybackService.mAudioFocusListener);
            }
            // case R.id.shuffle: {
            ...
            if (Util.isNexellSystem60()) { // 7.1 8.0 is not ok ,to do?
                ...
                if (mMediaPlayer != null) {
                    mMediaPlayer.setVisualizerStyle(mDisplayIndex,
                            mVisualizerType);
                }
                updateVisualizerView();
            }
	...
	private void updateNewActivity(int page) {
		...
		mHandler.post(new Runnable() {
			public void run() {
				Log.d("MusicPlayer", "updateNewActivity mSleepRestore:"
						+ mMediaPlayer.mSleepRestore);
				if (mMediaPlayer.mSleepRestore) {
					mMediaPlayer.mSleepRestore = false;
					Log.d("MusicPlayer",
							"updateNewActivity mMemoryPlayFromSleep:"
									+ mMediaPlayer.mMemoryPlayFromSleep);
		...
		if (list.size() > 0) { //
			mHandler.post(new Runnable() {
				public void run() {
					...
					// if (mPlayingTVList.getAdapter() == null) {
					updateListViewAdapter(mPlayingTVList, mMediaPlayer
							.getStrList(MusicPlayer.LIST_PLAYING, -1), 0);
					updateView();
				}
			});
		}
	...
	public void onResume() {
		...
		if (mDisplayIndex == 1) {
			...
			Log.d("MusicPlayer", mStartActivityNewPage + "zzzz" + mFirstPowerOn
					+ ":" + mMediaPlayer);
			if (mStartActivityNewPage == 0) {
				int delay = 50;
				if (Util.isRKSystem()) {
					if (mFirstPowerOn == 2 && mMediaPlayer != null) {
						int page = mMediaPlayer
								.getData(ComMediaPlayer.SAVE_DATA_PAGE);
						// Log.d("MusicPlayer", page+"zzzz");
						if (page == ComMediaPlayer.LIST_USB3
								|| page == ComMediaPlayer.LIST_USB4) {
							delay = 1800;
		...
		public void handleMessage(Message msg) {
			...
			case ComMediaPlayer.ID3_TIME_CHANGE:
				...
				if (MachineConfig.VALUE_SYSTEM_UI45_8702_2.equals(GlobalDef
						.getSystemUI())) {
					if (mPlayingTVList != null) {
						MyListViewAdapter ad = (MyListViewAdapter) mPlayingTVList
								.getAdapter();
						ArrayList<String> al = mMediaPlayer.getId3Time();
						ad.updateId3Time(al);
					}
	...
	private RelativeLayout.LayoutParams mToastParams = new RelativeLayout.LayoutParams(
	...
	public void autoPlayGpsRunAfterSleep(int status, Object obj) {
		...
		mIMediaCallBack.sendMessageDelayed(mIMediaCallBack.obtainMessage(
				MusicPlayer.STORAGE_MOUNTED, status, 0, obj), 20);
	...
	private void doAddExternalFiles() {
		...
		if ((i & 0x40000000) != 0) {
			...
			mCurrentList = mPlayingTVList;
			updateListViewAdapter(mPlayingTVList,
					mMediaPlayer.getStrListEx(MusicPlayer.LIST_PLAYING, -1), 0);
		}
	}
	...
	public void addExternalFiles(String files) {
		...
		if (!f.exists()) {
			Toast.makeText(mContext,
					mContext.getString(R.string.file_not_found) + " " + files,
