...
public class LrcView extends LrcViewBase {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (index != -1) {
            if (lines == 3 && index > 1) canvas.drawText(lrcpac.list.get(index - 1).lrcBody, centerPX, centerPY, pnPaint);

            canvas.drawText(lrcpac.list.get(index).lrcBody, centerX, centerY, cPaint);

            if (lines == 3 && index + 1 < lrcpac.list.size()) canvas.drawText(lrcpac.list.get(index + 1).lrcBody, centerNX, centerNY, pnPaint);
        }
