...
public class DVideoSpec {
    ...
    private native final int nativeSendDVSCommand(int cmd, int param1, int param2);
    ...
    public int sendDVSSetMemoryCommand() {
        if (TQ) {
            return sendDVSCommand(DVS_COMMON_CMD_1_PARAM, 0x1000);
        } else {
            return sendDVSCommand(DVS_COMMON_CMD_1_PARAM, DVS_SET_MEMORY);
        }
    }
    ...
    public void notifyLauncherInfo() {
        if (GlobalDef.mSource == MyCmd.SOURCE_DVD && MachineConfig.VALUE_SYSTEM_UI21_RM12.equals(GlobalDef.getSystemUI())) {
            if (GlobalDef.mContext != null) {
                ...
                i.putExtra(MyCmd.EXTRA_COMMON_DATA, (!(mPlayStatus == 3 || mPlayStatus == 0 || mPlayStatus == 4 || mPlayStatus == 5 || mPlayStatus == 13)) ? 1 : 0);

                i.putExtra(MyCmd.EXTRA_COMMON_DATA2, mDvdSecond + mDvdMinute * 60 + mDvdHour * 3600);
