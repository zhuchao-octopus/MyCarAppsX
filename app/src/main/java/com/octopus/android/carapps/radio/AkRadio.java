...
public class AkRadio {
	...
	public AkRadio(Context c) {
		...
		if (MachineConfig.VALUE_SYSTEM_UI20_RM10_1.equals(GlobalDef
				...
				|| MachineConfig.VALUE_SYSTEM_UI21_RM10_2.equals(GlobalDef
						.getSystemUI())
				|| MachineConfig.VALUE_SYSTEM_UI35_KLD813_2.equals(GlobalDef
						.getSystemUI())
				|| MachineConfig.VALUE_SYSTEM_UI21_RM12.equals(GlobalDef
						.getSystemUI())
				|| MachineConfig.VALUE_SYSTEM_UI45_8702_2.equals(GlobalDef
						.getSystemUI())
				|| MachineConfig.VALUE_SYSTEM_UI44_KLD007.equals(GlobalDef
						.getSystemUI())) {
			mFMNum = 3;
		}
	}
	...
	public int sendMRDCommand(int value, int param) {
		...
		case MRD_WRITE_FREQENCY:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					...
					(param & 0xff00) >> 8, param & 0xff);
			GlobalDef.setSource(mContext, RadioUI.SOURCE);
			break;
		case MRD_RDS_PTY_TYPE:
			BroadcastUtil.sendToCarServiceMcuRds(mContext,
					ProtocolAk47.SEND_RDS_SUB_OPT, 0x1, param);
			break;
		case MRD_SAVE:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 0x4, param);

			break;

		case MRD_LIST_INDEX:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 0x3, param);
	...
	public int sendMRDCommand(int value) {
		...
		case MRD_AS:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 1, 0);
			break;
		case MRD_PS:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 1, 1);
			break;
		case MRD_SCAN:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 1, 2);
			break;
		case MRD_STEP_UP:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 1, 5);
			break;
		case MRD_STEP_DOWN:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
		...
		case MRD_PREV_LIST_CHANNEL:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 1, 8);
			break;
		case MRD_NEXT_LIST_CHANNEL:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 1, 9);
			break;
		// BroadcastUtil.sendToCarServiceMcuRadio(mContext,
		// ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 1, 6);
		// break;
		case MRD_WRITE_BAND:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 0, mMRDBand);
			break;
		case MRD_WRITE_BAND2:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 0, 0xff);
			break;
		case MRD_WRITE_LOCDX:

			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 2,
					(mMRDFlag & MRD_INFO_FLAG_LOCDX) != 0 ? 1 : 0);
		...
		case MRD_RDS_AF_ON:
			BroadcastUtil.sendToCarServiceMcuRds(mContext,
					ProtocolAk47.SEND_RDS_SUB_OPT, 0, 1);
			break;
		case MRD_RDS_AF_OFF:
			BroadcastUtil.sendToCarServiceMcuRds(mContext,
					ProtocolAk47.SEND_RDS_SUB_OPT, 0, 0);
			break;
		case MRD_QUERY_RDS_INFO:
			BroadcastUtil.sendToCarServiceMcuRds(mContext,
					ProtocolAk47.SEND_RDS_SUB_QUERY, 0);
			break;
		case MRD_RDS_TA_ON:
			BroadcastUtil.sendToCarServiceMcuRds(mContext,
					ProtocolAk47.SEND_RDS_SUB_OPT, 0, 3);
			break;
		case MRD_RDS_TA_OFF:
			BroadcastUtil.sendToCarServiceMcuRds(mContext,
					ProtocolAk47.SEND_RDS_SUB_OPT, 0, 2);
			break;
		case MRD_RDS_ST_ON:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 0x5);
			break;
		case MRD_RDS_ST_OFF:
			BroadcastUtil.sendToCarServiceMcuRadio(mContext,
					ProtocolAk47.SEND_RADIO_SUB_RADIO_OPERATION, 0x6);
			break;
		default:
			cmd = false;
			break;
			...
			case ProtocolAk47.TYPE_RADIO_RECEIVE: {
				...
				case ProtocolAk47.RECEVE_RADIO_SUB_RADIO_CURRENT_INFO:
					if (buf.length > 11) {
						...
						Log.d(TAG, Integer.toHexString(((int) mMRDFlag & 0xff))
								+ ":" + mMRDFreqs[0] + ":" + mFreqMin);
...
//						Log.d("kkk", i + ":" + mMRDFreqs[i]);
			...
			case ProtocolAk47.TYPE_RDS_RECEIVE: {
				...
				case ProtocolAk47.RECEVE_RDS_SUB_PS_INFO:
					if (buf.length >= 10) {
						Util.byteArrayCopy(mMRDRdsPs, buf, 0, 2,
								mMRDRdsPs.length);
						if (mMRDRdsPs[0] != 0) {
	...
	public void reportCanboxPlayStatus() {
		...
		BroadcastUtil.sendCanboxInfo(mContext, MyCmd.RADIO_SOURCE_CHANGE,
				(int) mMRDFreqency, mMRDBand, curChannel);
