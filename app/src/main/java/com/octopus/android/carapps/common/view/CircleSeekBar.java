...
public class CircleSeekBar extends SeekBar {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (isHasCache) {
            ...
            canvas.drawArc(new RectF(left, top, right, bottom), -225, 270, false, mWheelPaint);
        }

        Log.d("ccc", "mCurProcess" + mCurProcess + ":" + mCurAngle);
        // 画选中区域
        if (mCurAngle < 225) {
            canvas.drawArc(new RectF(left, top, right, bottom), -225, (float) (mCurAngle + 360 - 225), false, mReachedPaint);
        } else {
            canvas.drawArc(new RectF(left, top, right, bottom), -225, (float) (mCurAngle - 225), false, mReachedPaint);
        }
