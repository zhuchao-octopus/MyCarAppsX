 ...
 */
public class CircleSeekBar extends androidx.appcompat.widget.AppCompatSeekBar {
	...
	protected void onDraw(Canvas canvas) {
		...
		if (isHasCache) {
			...
			canvas.drawArc(new RectF(left, top, right, bottom), -225, 270,
					false, mWheelPaint);
		}

		Log.d("ccc", "mCurProcess" + mCurProcess+":"+mCurAngle);
		// 画选中区域
		if (mCurAngle < 225) {
			canvas.drawArc(new RectF(left, top, right, bottom), -225,
					(float) (mCurAngle + 360 - 225), false, mReachedPaint);
		} else {
			canvas.drawArc(new RectF(left, top, right, bottom), -225,
					(float) (mCurAngle - 225), false, mReachedPaint);
		}
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (isCanTouch
			...
			mCurProcess = getSelectedValue(curAngle);
			Log.d("aaa", "mCurProcess" + mCurProcess+":"+mCurAngle);
			if (mCurProcess == 0 || mCurProcess == mMaxProcess){
				return false;
			}
			mCurAngle = curAngle;
	...
	private float calcXLocationInWheel(double angle, double cos) {
		if (angle < 180) {
			return (float) (getMeasuredWidth() / 2 + Math.sqrt(1 - cos * cos)
					* mUnreachedRadius);
		} else {
			return (float) (getMeasuredWidth() / 2 - Math.sqrt(1 - cos * cos)
					* mUnreachedRadius);
	...
	private int getSelectedValue(double curAngle) {
//		Log.d("ffck", ""+curAngle);
		
		if (curAngle >= 0 && curAngle <= 135) {
			return Math.round(mMaxProcess * ((float) ((curAngle + 135)/ 270)));
		} else if (curAngle >= 225 && curAngle <= 360) {
			return Math.round(mMaxProcess * ((float) ((curAngle - 225)/ 270)));
...
//		Log.d("eef", ":"+progress);
		progress *= 2;
		if (progress <= 1000) {
			progress = (progress*1500/2000);
			progress = (1250 + progress);
		} else {
			progress = (progress*1500/2000);
			progress -= 750;
		}
