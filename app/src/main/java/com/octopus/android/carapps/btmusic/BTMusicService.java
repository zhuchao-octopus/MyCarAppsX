            ...
            case MyCmd.Cmd.SOURCE_CHANGE:
                ...
                if (source != BTMusicUI.SOURCE) {
                    ...
                    if (UtilCarKey.mBtMusicInBTapk) {
                        GlobalDef.reactiveSource(mContext, BTMusicUI.SOURCE, BTMusicService.mAudioFocusListener);
                    }
                }
                break;
        }
        ...
        switch (cmd) {
            case MyCmd.Cmd.BT_SEND_A2DP_STATUS: {
                ...
                if (GlobalDef.mAutoTest) {
                    BroadcastUtil.sendToCarService(mContext, MyCmd.Cmd.AUTO_TEST_RESULT, MyCmd.SOURCE_BT, (mBtName == null) ? 1 : 0);
                }

                if (play != mPlayStatus) {

                    if (BTMusicUI.mUI[0] != null && (!BTMusicUI.mUI[0].mPause || mGpsRunAfter) && play == A2DP_INFO_CONNECTED && mPlayStatus < A2DP_INFO_CONNECTED) {
                        doKeyControl(MyCmd.Keycode.PLAY);
                        GlobalDef.reactiveSource(mContext, BTMusicUI.SOURCE, BTMusicService.mAudioFocusListener);
                        mGpsRunAfter = false;
                    } else {
                        if (mGpsRunAfter && GlobalDef.mSource == BTMusicUI.SOURCE && play == A2DP_INFO_CONNECTED && mPlayStatus < A2DP_INFO_CONNECTED) {
                            doKeyControl(MyCmd.Keycode.PLAY);
                            GlobalDef.reactiveSource(mContext, BTMusicUI.SOURCE, BTMusicService.mAudioFocusListener);
                            mGpsRunAfter = false;
                        }
